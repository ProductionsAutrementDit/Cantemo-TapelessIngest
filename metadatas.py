            ...
            if len(metadata_elements) is 1:
                if return_type is 'text':
                    if isinstance(metadata_elements[0], etree._Element):
                        ...
                        log.info("Unknown value type for %s (%s)" % (metadata_path, type(metadata_elements[0])))
                        return False
                    log.info("Map value %s to %s" % (value, metadata_path))
                else:
                ...
                return False
                log.info("No metadata found for %s" % metadata_path)
