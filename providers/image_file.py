...
class Provider(VideoFileProvider):
    ...
    def getFilesInfos(self, folder):
        ...
        if os.path.isfile(self.card_xml_file):
            ...
            files = [ f for f in os.listdir(path) if (f.lower().endswith(self.file_extensions) and (not f.startswith('.'))) ]
    ...
    def createClipFromFileWithSidecar(self, folder, material, reel):
        path = folder.path
    ...
    def createClipFromFile(self, folder, material, reel):
        ...
        if len(video_streams) > 0:
            ...
            try:
                video_timecode = video_stream.findall("tag[@key='timecode']")[0].get("value")
        ...
        if len(data_streams) > 0:
            ...
            try:
                data_timecode = data_stream.findall("tag[@key='timecode']")[0].get("value")
            except Exception as e:
                data_timecode = False
            try:
        ...
        if manufacturer:
            self.update_or_create_metadata(clip, 'device_manufacturer', manufacturer)
        ...
        InputFile = clip.input_files.create(filetype = file_type, path = os.path.join(path, format.get('filename')))
